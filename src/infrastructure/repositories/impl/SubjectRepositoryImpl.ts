...
export class SubjectRepositoryImpl implements SubjectRepository {
    ...
    async findAllStudents(subjectId: number): Promise<Student[]> {
        ...
        await Promise.all(studentSubjects.map(async (s) => {
            const result = await StudentDb.findByPk(s.studentId).catch((error) => {
                console.error('Erro ao procurar registro: ', error);
                throw new DatabaseError('Erro de banco de dados ao procurar registro');
            });
    ...
    async delete(id: number): Promise<boolean> {
        ...
        if (students.length != 0) {
            ...
            throw new ForeignKeyConstraintError('A matéria possui alunos matriculados');
        }
    ...
    private async findAllStudentsIds(subjectId: number): Promise<StudentSubject[]> {
        logger.info('SubjectRepository findAllStudents');
