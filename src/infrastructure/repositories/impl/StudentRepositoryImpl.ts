...
import { studentDbArrayIntoStudentArray, studentDbIntoStudent } from '../../db/utils/StudentDbUtils';
...
export class StudentRepositoryImpl implements StudentRepository {
    ...
    async findSubjectsById(id: number): Promise<Subject[]> {
        ...
        const studentSubjects: StudentSubject[] = await this.findAllStudentSubjects(id);

        const subjects: Subject[] = [];
        await Promise.all(studentSubjects.map(async (s) => {
            const subject: Subject = await this.findSubjectById(s.subjectId);
        ...
        try {
            const [rowsUpdated] = await StudentDb.update(updatedStudent, {
                ...
                if (error.name === 'SequelizeForeignKeyConstraintError') {
                    console.error('Foreign key error: ', error);
                    throw new ClassNotFoundError(`Turma com id ${updatedStudent.classId} não encontrada`);
                } else {
            ...
            if (rowsUpdated > 0) {
                ...
                if (updatedModel) {
                    ...
                    throw new StudentNotFoundError('Estudante não encontrado após a atualização');
                }
            } else {
                logger.error('Student not found');
                throw new StudentNotFoundError('Estudante não encontrado');
    ...
    private async findAllStudentSubjects(studentId: number): Promise<StudentSubject[]> {
        ...
        const results = await StudentSubjectDb.findAll(whereClause).catch((error) => {
            console.error('Erro ao procurar registros: ', error);
            throw new DatabaseError('Erro de banco de dados ao procurar registros');
        });

        const studentSubjects: StudentSubject[] = studentSubjectDbArrayIntoStudentSubjectArray(results);
    ...
    private async addStudentSubject(studentId: number, subjectId: number): Promise<boolean> {
        ...
        try {
            ...
            if (error instanceof ForeignKeyConstraintError) {
                ...
                throw new DuplicateRecordError('Aluno já está cadastrado nessa disciplinas');
            } else {
                console.error('Erro ao criar registro: ', error);
                throw new DatabaseError('Erro de banco de dados ao criar registro');
            }
    ...
    private async deleteStudentSubject(id: number, subjectId: number): Promise<boolean> {
